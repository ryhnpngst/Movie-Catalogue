...
const FooterToolsInitiator = {
  ...
  async _subscribePushMessage(event) {
    ...
    if (await this._isCurrentSubscribetionAvailable()) {
  ...
  async _isCurrentSubscribetionAvailable() {
  ...
  async _isNotificationReady() {
    ...
    if (!NotificationHelper._checkPermission()) {
      ...
      if (status === 'denied') {
        window.alert('Cannot subscribe to push message because of notification permission is denied');
        return false;
      }

      if (status === 'default') {
        window.alert('Cannot subscribe to push message because of notification permission is ignored');
        return false;
      }
    }
